...
class SentimentAgent:
    ...
    async def _get_lunarcrush_sentiment(self):
        """LunarCrush API - Nonaktif (perlu subscription)"""
        return {"signal": "HOLD", "confidence": 50, "score": 0, "source": "LunarCrush (disabled)"}
    ...
    def _combine_signals(self, fg, guavy, free_news, alpha, lunarcrush=None):
        ...
        # LunarCrush (bobot 2)
        if lunarcrush:
            lunarcrush_signal = lunarcrush.get('signal', 'HOLD')
            if lunarcrush_signal in signals:
                signals[lunarcrush_signal] += 2
