...
class RiskAgent:
    ...
    async def _get_volume_ratio_1d(self):
        """Ambil volume ratio dari timeframe 1d langsung dari Yahoo Finance"""
        try:
            ...
            print(f"📊 [RISK] Volume ratio (1d langsung dari Yahoo): {volume_ratio}x")
            return volume_ratio
        except Exception as e:
            print(f"⚠️ [RISK] Gagal ambil volume ratio: {e}")
            return 1.0

    async def analyze(self, balance, has_position, current_position=None, atr_percent=None, volume_ratio=None, rsi=None, macd_trend=None, bb_signal=None):
        # Ambil volume ratio dari Yahoo Finance langsung (timeframe 1d)
        volume_ratio_1d = await self._get_volume_ratio_1d()
        # Gunakan volume_ratio_1d (override parameter dari technical agent)
        ...
        if volume_ratio is not None:
            ...
            print(f"📊 [RISK] Volume ratio (1d dari Yahoo): {volume_ratio}x (threshold: {self.min_volume_ratio})")
            if volume_ratio < self.min_volume_ratio:
                result["can_trade"] = False
                result["risk_score"] = 100
                result["reason"] = f"Low volume: {volume_ratio:.1f}x (need >= {self.min_volume_ratio})"
                print(f"   ❌ [RISK] BLOCKED: Volume too low")
                return result
            else:
                print(f"   ✅ [RISK] Volume OK")
